...
| Maintenance write amplification | Authoritative routing can keep one durable owner per fact, so small repo changes do not require agents to reread or rewrite several harness docs. | If every code change forces updates across state files, evidence logs, manifests, roadmap, and public docs, the harness can spend more context and editing work than it saves. | Track docs fan-out, harness-file edits, doc-edit tokens, review time, and stale-duplicate repairs as first-class costs in profile comparisons. |
